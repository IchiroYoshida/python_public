...
def plotCell(self,ax):
    for x in range(self.Mesh):
        for y in range(self.Mesh):
            ...
            for i in range(len(cell[3])):    #U: Unsusceptible
                plotPerson(self,ax, x, y, 'yellow')
...
class Person:
    def __init__(self):
        ...
        self.x = 0
        self.y = 0
        self.condition = 0 #[S, I, R, U]  U:unsusceptible
...
class Infection(object):
    ...
    def unsusceptiblePerson(self):
        unsus_N = int(self.N * self.unsus /100.)
        for i in range(0,unsus_N):
            self.person[i].id = i
            self.person[i].condition = 3
    ...
    def dailyReport(self):
        ...
        for n in range(self.N):
            if(self.person[n].condition == 3): # Unsusceptible
                u.append(n)

            elif(self.person[n].condition == 0): # Susceptible
                s.append(n)
