...
Icon_PushPinY = CustomIcon(icon_image="./ylw-pushpin.png",
                icon_anchor=(15,15),
                popup_anchor=(0,0))

Icon_Sailing = CustomIcon(icon_image="./sailing.png",
                icon_size=(30,30),
                icon_anchor=(15,15),
                popup_anchor=(0,0)) 

Icon_Exit = CustomIcon(icon_image="./track-none.png",
                icon_size=(30,30),
                icon_anchor=(15,15),
                popup_anchor=(0,0)) 
    ...
    for dat in data[1:]:
        ...
        if (MA <= MoonAge2 < (MA+1)):
            ...
            if (Style == 'D'):
                ...
                #Entry point
                #folium.Marker(location=Entry, popup=Name, icon=Icon_PushPinY).add_to(fmap1)
                folium.Marker(location=Entry,popup=desstr,icon=folium.Icon("orange")).add_to(fmap1)
                
                
                #Exit point
                #folium.Marker(location=Exit, icon=Icon_Exit).add_to(fmap1)

                #Sailing
                #folium.Marker(location=Mid, icon=Icon_Sailing, popup= desstr).add_to(fmap1)
