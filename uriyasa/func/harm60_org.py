"""""""""""""""""
海上保安庁 日本沿岸 潮汐調和定数６０分潮計算
by Ichiro Yoshida (yoshida.ichi@gmail.com)
2023-06-16
"""""""""""""""""
import math
import datetime
...
class Tide(object):
   def __init__(self,td3,date):
      self.itv = 20                 # 計算間隔20分
      self.name = td3.label['Name']
      self.lat  = td3.label['Latitude']
      self.lng  = td3.label['Longitude']
      self.level= td3.label['Level']
      self.date = date
      self.hr   = td3.harms['hr'].values.tolist()
      self.pl   = td3.harms['pl'].values.tolist()
      ...
      self.ti = pd.Series(self.tide, index=pd.date_range(self.date+"  0:00", self.date+" 23:59", freq='min'))
