...
class Tide(object):
      def __init__(self,port):
         ...
         self.m2  = np.zeros(self.inc+3,np.float64)   #M2分潮
         self.s2  = np.zeros(self.inc+3,np.float64)   #S2分潮
         self.m2s2 = np.zeros(self.inc+3,np.float64)   #M2+S2 分潮
      ...
      def wav(self,port):
         ...
         for i in range(0,self.inc+3):
           ...
           #self.m2[i] = self.level     #M2
           #self.s2[i] = self.level
           ...
           #M2+S2
           self.m2s2[i]  = self.m2[i]+self.s2[i]
         ...
         #M2分潮の１分毎の潮位を算出
         range_m2= -40+20*len(self.m2)
         func_levelm2 = interp1d(time,self.m2,kind='cubic')
         func_tidem2  = func_levelm2(func_time)
         self.tidem2 =func_tidem2[40:1480]

         #M2  干潮と満潮
         self.hitidem2, self.lowtidem2 = flood_ebb(func_time,func_tidem2)

         #S2分潮の１分毎の潮位を算出
         range_s2 = -40+20*len(self.s2)
         func_levels2 = interp1d(time,self.s2,kind='cubic')
         func_tides2  = func_levels2(func_time)
         self.tides2  = func_tides2[40:1480]

         #M2S2
         func_tidem2s2 = func_levelm2(func_time)+func_levels2(func_time)
         self.tidem2s2 = func_tidem2s2[40:1480]
         self.hitidem2s2, self.lowtidem2s2 = flood_ebb(func_time,func_tidem2s2)
