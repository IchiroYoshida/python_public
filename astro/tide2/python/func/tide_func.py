"""
暦：太陽と月の出没時刻、月齢、潮名、干満時刻（６０分潮計算）

2018/01/07  Ichiro Yoshida
"""
import math
import datetime
import numpy as np
...
"""
   ...
   hitide, lowtide = flood_ebb(time,level,delta)
...
def flood_ebb(time,level,delta):
   ...
   # 満潮処理
   hitide_time0   = np.array(hitides0)[:,0]-delta #時間、計算開始はdelta分前から
   ...
   # 干潮処理
   lowtide_time0  = np.array(lowtides0)[:,0]-delta
...
class Tide(object):
      def __init__(self,port):
         ...
         self.hr   = port.hr
         self.pl   = port.pl
         self.inc = int(24*60/self.itv)+2   #１日（２４時間x６０分／計算間隔）の計算回数
         self.tl  = np.zeros(self.inc+3,np.float64)
         
         # 主要４分潮（M2,S2,O1,K1）
         self.m2 = np.zeros(self.inc+3,np.float64)
         self.s2 = np.zeros(self.inc+3,np.float64)
         self.o1 = np.zeros(self.inc+3,np.float64)
         self.k1 = np.zeros(self.inc+3,np.float64)
         self.t4 = np.zeros(self.inc+3,np.float64)
      ...
      def wav(self,port):
         ...
         for i in range(0,self.inc+3):
           ...
           #M2分潮算出
           self.m2[i] += f[30] * self.hr[30] * math.cos((vl[30] \
                      + ags[30] * (i - 2) / (60 /self.itv) \
                      - self.pl[30]) * lib.dr)

           #S2分潮算出
           self.s2[i] += f[35] * self.hr[35] * math.cos((vl[35] \
                      + ags[35] * (i -2) / (60 /self.itv) \
                      - self.pl[35]) * lib.dr)

           #O1分潮算出
           self.o1[i] += f[9] * self.hr[9] * math.cos((vl[9] \
                      + ags[9] * (i -2) / (60 /self.itv) \
                      - self.pl[9]) * lib.dr)

           #K1分潮算出
           self.k1[i] += f[16] * self.hr[16] * math.cos((vl[16] \
                      + ags[16] * (i -2) / (60 /self.itv) \
                      - self.pl[16]) * lib.dr)
           
           #T4 = M2+S2+O1+K1
           self.t4[i] = self.m2[i] + self.s2[i]  + self.o1[i] + self.k1[i]
         ...
         new_time = np.linspace(time0.min(), time0.max(), len_tl)
         func_level = interp1d(new_time,tl,kind='cubic')

         func_time = np.arange(-self.itv*2,range_tl+self.itv*2+1,1)
         func_tide = func_level(func_time)

         #満潮と干潮（時刻：分、潮位）
         self.hitide, self.lowtide = flood_ebb(func_time,func_tide,self.itv*2)
         
         self.tide = func_tide[2*self.itv:range_tl+2*self.itv]

         #補間し１分毎の主要４分潮合成波を算出
         func_level_t4 = interp1d(new_time, self.m2, kind='cubic')
         func_tide_t4  = func_level_t4(func_time)
         self.tide_t4  = func_tide_t4[40:1480]

         #主要４分潮合成波の干潮と満潮
         self.hitide_t4, self.lowtide_t4 = flood_ebb(func_time, func_tide_t4, self.itv*2)
