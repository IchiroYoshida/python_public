...
class CurrentTable(object):
    ...
    def day_show(year, month, day, observer):

        observer.date = str(year)+str('/%02d' % month)+str('/%02d' % day)

        print('\n\n', observer.name, year, month, day)
    ...
    def month_show(year, month, observer):
        days = tf.month_days(year, month)

        print('\n\n',observer.name, year,month)
...
#CurrentTable.day_show(2018, 6, 1,td)

CurrentTable.month_show(2013, 5, td)
