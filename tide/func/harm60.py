...
class Tide(object):
   def __init__(self,td3,date):
      itv = 20                               # 計算間隔20分
      name = td3.label['Name']               # 地点名
      lat  = td3.label['Latitude']           # 緯度
      lng  = td3.label['Longitude']          # 経度
      level= td3.label['Level']              # 基準
      hr   = td3.harms['hr'].values.tolist() #調和定数 振幅
      pl   = td3.harms['pl'].values.tolist() #調和定数 遅角
      inc = int(24*60/itv)+2   #１日（２４時間x６０分／計算間隔）の計算回数
      tl  = np.zeros(inc+3,np.float64)
      ...
      for i in range(0,inc+3):
         tl[i] = level
         #tl = [float(level) for i in range(0,inc+3)]
      ...
      time       = sorted(np.arange(-itv*2,range_tl+3*itv,itv))
      time0  = np.array(time)

      new_time = np.linspace(time0.min(), time0.max(), len_tl)
      func_level = interp1d(new_time,tl,kind='cubic')

      func_time = np.arange(-itv*2,range_tl+itv*2+1,1)
