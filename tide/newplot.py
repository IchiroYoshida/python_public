...
t2 = pt.tide[(date+"  6:00"):(date+"  18:00")]

eb2 = pt.ebb[(date+"  6:00"):(date+"  18:00")]
fl2 = pt.flow[(date+"  6:00"):(date+"  18:00")]
...
Tmin = datetime.datetime.strptime(date+"  5:45",'%Y/%m/%d %H:%M')
Tmax = datetime.datetime.strptime(date+" 18:15",'%Y/%m/%d %H:%M')
...
#06:00 Left
ti = datetime.datetime.strptime(date+ "   6:00",'%Y/%m/%d %H:%M')
hi = t2[(date + "  6:00")]
StrLevel ='{:.0f}'.format(hi)+' (cm)'
ax.text(ti, hi-offset, StrLevel)

#18:00 Right
ti = datetime.datetime.strptime(date+ "  18:00",'%Y/%m/%d %H:%M')
hi = t2[(date + " 18:00")]
StrLevel ='{:.0f}'.format(hi)+' (cm)'
ax.text(ti, hi-offset, StrLevel)
